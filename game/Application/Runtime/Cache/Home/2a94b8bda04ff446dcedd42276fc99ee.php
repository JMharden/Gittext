...
<!-- <script src="http://m.dqpps.com/js/wx.js"></script> -->
		...
		<script>
			function payinner(){
				window.location = '/kpan.php?m=&c=Index&a=liveAccredit&money=' + $("#money").val();
			}
			function rechar() {
				$(".toast3").hide();
				$(".chou").trigger("click");
				...
				$('.chou').click(function() {
					...
					if(st == '0') {
						...
						if(userm < money2 * 1 && pid == '0') { 
							...
							$("#pid").val('0');
							$.post('/kpan.php?m=&c=Index&a=paybuy&tid=' + pan2 + "&money=" + money2 + "&zhuan" + zhuan + '&pid=' + pid, {}, function(data) {
								if(data.status == 1) {
		...
		<script type="text/javascript">
			function goPayment(money) {
				...
				if(ispay == 0) {
					$(".ispay").val(1);
					$.post("/kpan.php?m=&c=Index&a=payhao", { money: money }, function(d) {
						$(".ispay").val(0);
						if(d.status == 1) {
