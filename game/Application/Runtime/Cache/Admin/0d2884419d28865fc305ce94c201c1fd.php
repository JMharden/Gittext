	...
	<body>
				...
				<ul class="sidebar-menu">
					...
					<li class="treeview" onclick="index(5)">
						...
						<ul class="treeview-menu">
							...
							<li>
								<a href="/kpan.php?m=Admin&c=User&a=feedback"><i class="fa fa-circle-o"></i>用户反馈</a>
							</li>
        ...
        <div id="contentwrapper" class="contentwrapper lineheight21">
			<div class="tableoptions">        
				<form method="post">
					用户ID:
					<input type="text" name="userid" value="<?php echo ($_GET['userid']); ?>" class="smallinput" style="width:100px;" />
					订单号:
					<input type="text" name="order_sn" value="<?php echo ($_GET['order_sn']); ?>" class="smallinput" style="width:180px;" />
					预支付订单号:
					<input type="text" name="prepay_id" value="<?php echo ($_GET['prepay_id']); ?>" class="smallinput" style="width:320px;" />
					支付状态：
			...
			<table cellpadding="0" cellspacing="0" border="0" id="table2" class="stdtable stdtablecb">
				<thead>
					<tr>
						<th class="head1">ＩＤ</th>
						<th class="head1">用户ＩＤ</th>
						<th class="head1">订单号</th>
						<th class="head1">预支付订单号</th>
						<th class="head0">充值金额</th>
						<th class="head0">获得金币</th>
						<th class="head0">支付状态</th>
						<th class="head0">记录时间</th>
				...
				<tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><?php echo ($vo["golds"]); ?></td>
						<td><?php if($vo['status'] == 1): ?>未支付<?php elseif($vo['status'] == 2): ?>已支付<?php else: ?>--<?php endif; ?>
						</td>
						<td><?php if($vo['ctime'] > 0): echo (date("Y-m-d H:i:s",$vo["ctime"])); else: ?>--<?php endif; ?></td>
						<!-- <td><?php if($vo['uptime'] > 0): echo (date("Y-m-d H:i:s",$vo["uptime"])); else: ?>--<?php endif; ?></td> -->
