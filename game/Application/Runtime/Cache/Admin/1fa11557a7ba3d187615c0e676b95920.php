...
<html lang="zh">
	...
	<body>
		...
		<div class="centercontent">
			        <script src="/Public/plugins/My97DatePicker/WdatePicker.js"></script>        <div class="pageheader notab">            <h1 class="pagetitle">添加/编辑域名信息</h1>        </div><!--pageheader-->        <div id="contentwrapper" class="contentwrapper lineheight21">			<form class="stdform stdform2" method="post">				<p>					<label>ID</label>					<span class="field">						<input type="hidden" name="id" id="id" value="<?php echo ($info["id"]); ?>" class="smallinput" />						<input type="text" name="id" id="id" value="<?php echo ($info["id"]); ?>" disabled class="smallinput" />					</span>				</p>				<p>					<label>域名</label>					<span class="field">						<input type="text" name="domain" id="domain" value="<?php echo ($info["domain"]); ?>" class="smallinput" />					</span>				</p>				<p>					<label>是否黑名单</label>					<span class="field">						<input type="radio" name="is_lock" id="is_lock" value="1"  <?php if($info["is_lock"] == 1): ?>checked<?php endif; ?> />						&nbsp;&nbsp;是&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;                        <input type="radio" name="is_lock" id="is_lock" value="0"  <?php if(!isset($info['is_lock'])): ?>checked<?php endif; ?> <?php if($info["is_lock"] == 0): ?>checked<?php endif; ?> />                        &nbsp;&nbsp;否					</span>				</p>				<?php if(!isset($_GET['type'])): ?><p>					<label>是否默认域名</label>					<span class="field">						<input type="radio" name="is_home" id="is_home" value="1"  <?php if($info["is_home"] == 1): ?>checked<?php endif; ?> />						&nbsp;&nbsp;是&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;                        <input type="radio" name="is_home" id="is_home" value="0"  <?php if(!isset($info['is_home'])): ?>checked<?php endif; ?> <?php if($info["is_lock"] == 0): ?>checked<?php endif; ?> />                        &nbsp;&nbsp;否					</span>				</p><?php endif; ?>				<!-- <p>					<label>是否二维码域名</label>					<span class="field">						<input type="radio" name="is_qr_code" id="is_qr_code" value="1" <?php if($info["is_qr_code"] == 1): ?>checked<?php endif; ?> />						&nbsp;&nbsp;是&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;				                        <input type="radio" name="is_qr_code" id="is_qr_code" value="0"  <?php if(!isset($info)): ?>checked<?php endif; ?> <?php if($info["is_qr_code"] == 0): ?>checked<?php endif; ?> />				                        &nbsp;&nbsp;否					</span>				</p> -->				<p class="stdformbutton">					<button class="submit radius2">提交</button>					<input type="reset" class="reset radius2" value="重置" />					<?php if(!isset($_GET['type'])): ?><input type="hidden" name="is_type" value="0" />					<?php else: ?>						<input type="hidden" name="is_type" value="1" /><?php endif; ?>				</p>			</form>        </div><!--contentwrapper-->     
		</div>
	</body>
